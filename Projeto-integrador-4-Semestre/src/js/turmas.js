...
function atualizarListasTurmas(turmas) {
    ...
    const turmasHTML = turmas.map(turma => `
        <div class="table-row" data-id="${turma.id}">
            <a href="chamada.html?id=${turma.id}&nome=${encodeURIComponent(turma.nome)}" class="btn-chamada" title="Abrir chamada">
                <span class="material-symbols-outlined">groups</span>
            </a>
            <div class="table-data">
                <p class="table-row-title">${turma.nome}</p>
                <span class="table-badge">${turma.unidade}</span>
            </div>
...
// Função para abrir página de chamada
function abrirChamada(turmaData) {
    // TODO: Substituir por navegação real para página de chamada
    console.log('Abrindo chamada para turma:', turmaData);
    // Por enquanto, redireciona para uma página de chamada (será criada depois)
    window.location.href = `chamada.html?id=${turmaData.id}&nome=${encodeURIComponent(turmaData.nome)}`;
}

// Função para anexar event Listeners aos elementos dinâmicos
